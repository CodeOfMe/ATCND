...
ATCND 是一个模型无关的框架，通过将 K 值选择视为用户指定整数范围上的结构化搜索问题，来确定主题模型（LDA/NMF）的最优主题数或聚类方法（K-Means）的最优聚类数。相比穷举网格搜索需要 O(K_max - K_min) 次模型评估，ATCND 采用二分搜索、黄金分割搜索或三分搜索实现 O(log(K_max - K_min)) 次评估，同时匹配或超过网格搜索的质量。
